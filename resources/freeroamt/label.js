...
function initLabelScript() {
	console.log("[TRMPOSO.Label]: Initializing label script ...");
	...
	console.log("[TRMPOSO.Label]: Label script initialized!");
}
...
function renderPropertyEntranceLabel(name, position, locked, isBusiness, price, rentPrice, labelInfoType) {
	...
	if(!locked && labelInfoType != PROPLABEL_INFO_NONE) {
		let infoText = "";
		switch(labelInfoType) {
			case PROPLABEL_INFO_ENTER:
				if(enterPropertyKey) {
					...
					if(pickups[i].getData("vrr.label.type") != null) {
						if(getDistance(localPlayer.position, pickups[i].position) <= renderLabelDistance) {
							if(!pickups[i].isOnScreen) {
								let price = "0";
								let rentPrice = "0";
								let labelInfoType = PROPLABEL_INFO_NONE;
